...
main(){
...

    {
        ...
        for pkg in "${apt_packages[@]}"; do
            printf "[*] Instalando %s (APT)...\n" "$pkg"
            if apt install -y "$pkg" >/dev/null 2>&1; then
                printf "[✔] %s instalado correctamente.\n" "$pkg"
            else
                printf "[✖] Error al instalar %s.\n" "$pkg"
            fi
            sleep 0.3
        done
        echo
    ...
    } | dialog --title "Instalador de Stellar" --progressbox 20 70

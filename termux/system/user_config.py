...
EOF
            ...
            [bold blue][1][/bold blue] FiraMono
            
            """, title=translate(category="user-config", type="fonts_title_message"), border_style="bold cyan")
            tips_banner = Panel(translate(category="user-config", type="tips_message"), border_style="bold cyan")
            console.print(list_fonts)
            console.print(tips_banner)
            font_type = console.input(f"[bold green]{translate(category="user-config", type="enter_message")} [/bold green]")
            if font_type:
                if font_type == "FiraMono" or font_type == "1":
                    os.system(f"cd ~/Stellar/resources/fonts/fira-mono && command cp font.ttf ~/.termux")
                    subprocess.Popen(["termux-reload-settings"])
                    console.print(f"[bold green][SUCCESS][/bold green] {translate(category="user-config", type="success_message")}")
                    time.sleep(2)
                    banner()
                else:
                    console.print(f"[bold red][ERROR][/bold red] {translate(category="user-config", type="invalid_font_message")}")
                    time.sleep(2)
                    return
            else:
                console.print(f"[bold red][ERROR][/bold red] {translate(category="user-config", type="invalid_data_message")}")
            ...
            subprocess.Popen(['clear'])
            time.sleep(1)
            user = console.input(f"[bold green]{translate(category="user-config", type="enter_message")} [/bold green]")
            if user:
                os.system(f"cd ~/Stellar/configs && echo {user} > user.stel")
                time.sleep(2)
                console.print(f"[bold green][SUCCESS][/bold green] {translate(category="user-config", type="success_message")}")
                banner()
            else:
